...
# Use localhost for development to connect to the local brain
BRAIN_URL = os.getenv("NEXUS_BRAIN_URL", "http://localhost:3006")
AGENT_URL = f"{BRAIN_URL}/api/orchestrate"
...
class NexusOS(App):
    ...
    async def _run_agent_loop(self, user_input: str, log: Log) -> None:
        ...
        for iteration in range(MAX_AGENT_LOOPS):
            ...
            try:
                # Call the Master Brain (Sovereign Trust Channel)
                headers = {
                    "Authorization": f"Bearer {os.getenv('SUPABASE_SERVICE_KEY', '')}",
                    "Content-Type": "application/json"
                }
                # The production Node API uses 'input' and 'history' for the master brain
                payload = {
                    "input": user_input,
                    "history": self.history
                }
                resp = await asyncio.to_thread(
                    requests.post, f"{BRAIN_URL}/api/master/chat", json=payload, headers=headers, timeout=60
                )
