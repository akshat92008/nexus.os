...
import { isRedisAvailable } from '../utils/env.js';

const REDIS_URL = process.env.REDIS_URL;
let connection: Redis | undefined;

if (!REDIS_URL || !isRedisAvailable()) {
  logger.warn('[Queue] Redis URL missing or invalid for this environment. Background workers and durable queues will be DISABLED (In-Memory Mock Mode).');
} else {
  try {
