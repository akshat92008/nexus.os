 ...
 */
function getDomainGuidance(goalType: string): string {
...
function deduplicateAndScore(nodes: TaskNode[], goal: string): TaskNode[] {
  ...
  for (const candidate of nodes) {
    ...
    if (candidate.goalAlignment < 0.07 && candidate.priority === 'low') {
      console.log(
        `[Planner] ✂️ Dropped misaligned task: "${candidate.label}" ` +
...
function validateAndRepair(raw: unknown, goal: string): TaskDAG {
  ...
  for (const node of plan.nodes) {
    ...
    node.contextFields = node.contextFields.filter((f: string) =>
      node.dependencies.includes(f) && ids.has(f)
    );
  }
  ...
  let remaining = [...plan.nodes as TaskNode[]];
...
function buildFallbackPlan(goal: string): TaskDAG {
