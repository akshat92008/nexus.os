...
function resolveWindowType(goalType: GoalType, mode: string): ApplicationWindowType {
  if (mode === 'student') return 'learning_workspace';
  if (mode === 'developer') return 'code_studio';
...
export function computeExecutionWaves(nodes: TaskNode[]): TaskNode[][] {
  ...
  while (remaining.length > 0 && safetyLimit-- > 0) {
    ...
    if (wave.length === 0) {
      throw new Error(
        '[Orchestrator] Cannot resolve next execution wave — possible cycle or unresolvable dependencies. ' +
...
async function executeTask(
  ...
  // ── Strategic Checkpoint (HITL) ──────────────────────────────────────────
  if (task.requiresApproval) {
    console.log(`[Orchestrator] ✋  Task "${task.id}" requires approval. Strategic Pause for ${deps.sessionId}`);

    safeEmit(res, {
      type: 'awaiting_approval',
      taskId: task.id,
      taskLabel: task.label,
      reason: 'Strategic Checkpoint: User review required before proceeding.'
    }, isAborted);

    const approved = await approvalGuard.wait(deps.sessionId, task.id);
    if (!approved) {
      console.warn(`[Orchestrator] 🛑 Task "${task.id}" REJECTED by user.`);
      registry.markFailed(task.id, 'Task rejected by user');
      throw new Error(`Mission paused: Task "${task.label}" was rejected.`);
    }
    console.log(`[Orchestrator] ✅ Task "${task.id}" APPROVED. Resuming...`);
  }
  ...
  try {
    ...
    if (recursiveRunner.isComplex(task)) {
      ...
      // Standard Execution
      finalResult = await governor.execute(() =>
        runAgent({
          task,
          goal: dag.goal,
...
async function executeWave(
  ...
  const results = await Promise.allSettled(
    wave.map(async (task, i) => {
      if (i > 0 && wave.length > 1) await sleep(i * 500); // 500ms stagger per task (was 300)
  ...
  const failedCritical = wave.filter((task, i) =>
    results[i].status === 'rejected' && task.priority === 'critical'
  );
...
export async function orchestrateDAG(deps: OrchestratorDeps): Promise<void> {
  ...
  const mode = (dag as any).metadata?.mode || 'student';
  ...
  safeEmit(res, {
    ...
    tasks: {
      parallel: parallelTasks.map((n): SubTask => ({ id: n.id, label: n.label, agentType: n.agentType })),
      sequential: sequentialTasks.map((n): SubTask => ({ id: n.id, label: n.label, agentType: n.agentType })),
    },
  ...
  const retryable = failedTasks.filter((r) => {
    ...
    if (node.priority === 'critical') return false; // critical fails are not retried silently
    if (!registry.canRetry(r.taskId, node.maxRetries)) return false;
    ...
    const allDepsCompleted = node.dependencies.every(
      (dep) => registry.getStatus(dep) === 'completed'
    );
    if (!allDepsCompleted) {
  ...
  let missionWorkspace: any = undefined;
  ...
  if (!isAborted() && memory.size > 0) {
    try {
      ...
      const allArtifacts = new Map<string, any>();
      allEntries.forEach(e => allArtifacts.set(e.taskId, e.data));

      const isStudent = (dag as any).isStudentMode;

      if (isStudent) {
        const studentOutput = formatStudentOutput(synthesis);
        missionWorkspace = formatStudentToWorkspace(
          studentOutput,
          dag.goal,
          sessionId
        );
        ...
        missionWorkspace = formatFounderToWorkspace(
          founderOutput,
          dag.goal,
          sessionId
        );
        ...
        missionWorkspace = formatDeveloperToWorkspace(
          developerOutput,
          dag.goal,
          sessionId
        );
      } else {
        missionWorkspace = transformToWorkspace(
          synthesis,
          dag.goal,
          dag.goalType,
          sessionId,
...
export async function executeSingleAction(
  ...
  try {
    ...
    const updatedWorkspace = await nexusStateStore.completeNextAction(
      userId,
      workspaceId,
      actionId,
      `Successfully executed: ${result.title}`
    );
