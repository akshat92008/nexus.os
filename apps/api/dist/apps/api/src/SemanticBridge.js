...
export class SemanticBridge {
     ...
     */
    async synthesizeBriefing(goal, artifacts, targetTask) {
        console.log(`[SemanticBridge] ⚡ Synthesizing briefing for Task: ${targetTask.id}...`);
        if (artifacts.length === 0)
            return 'No prior context available. Start from scratch.';
        ...
        const rawContext = relevantEntries
            ...
            .join('\n\n');
        if (!rawContext)
            return 'No directly relevant prior context found for this task.';
        const prompt = `
...
//# sourceMappingURL=SemanticBridge.js.map