...
class MasterBrainV2 {
    ...
    loopInterval = null;
    globalReflectionInterval = null;
    // ── Mission Lifecycle ───────────────────────────────────────────────────────
    registerMission(missionId, workspaceId, goal, goalType = 'general') {
        const state = {
            missionId,
            workspaceId,
            goal,
    ...
    updateMissionStatus(missionId, status, dag) {
    ...
    depositArtifact(missionId, taskId, content) {
        const mission = this.state.missions.get(missionId);
        if (!mission)
            return;
    ...
    getActiveMissions() {
        return this.getAllMissions().filter(m => m.status === 'running' || m.status === 'planning');
    }
    // ── Next Action Scoring ─────────────────────────────────────────────────────
    scoreNextActions(workspaceId, rawActions) {
        const scored = rawActions.map(a => {
    ...
    detectOpportunities(mission) {
        const artifactCount = Object.keys(mission.artifacts).length;
        if (artifactCount < 2)
            return;
        const otherMissions = this.getAllMissions().filter(m => m.missionId !== mission.missionId && m.status === 'complete');
    ...
    `;
        try {
            ...
            const { newOpportunities } = JSON.parse(data.choices[0].message.content);
    ...
    // ── Decision Loop ───────────────────────────────────────────────────────────
    startDecisionLoop(intervalMs = 60_000) {
        ...
        this.loopInterval = setInterval(() => {
            ...
            const now = Date.now();
            this.state.globalActions = this.state.globalActions.filter(a => !a.expiresAt || a.expiresAt > now);
            console.log(`[MasterBrain] 🔄 Cycle #${this.state.decisionCycle} — ` +
                `${this.getActiveMissions().length} active missions, ` +
                `${this.state.globalActions.length} queued actions`);
        }, intervalMs);
    }
    startGlobalReflection(intervalMs = 3600_000) {
...
//# sourceMappingURL=masterBrain.js.map