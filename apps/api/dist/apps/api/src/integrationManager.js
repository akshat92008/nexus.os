...
export function grantPermission(workspaceId, toolId) {
...
export function revokePermission(workspaceId, toolId) {
    workspacePermissions.get(workspaceId)?.delete(toolId);
}
export function hasPermission(workspaceId, toolId) {
    return workspacePermissions.get(workspaceId)?.has(toolId) ?? false;
}
export function getWorkspaceTools(workspaceId) {
...
export function initWorkspacePermissions(workspaceId) {
...
export function queueApproval(toolId, params, workspaceId) {
    const approval = {
...
export function listPendingApprovals(workspaceId) {
    return Array.from(approvalQueue.values()).filter(a => a.status === 'pending' && (!workspaceId || a.workspaceId === workspaceId));
}
// ── Simulated Tool Implementations ───────────────────────────────────────────
const isSimulated = (envKey) => !process.env[envKey];
const TOOLS = [
    {
        id: 'send_email',
        name: 'Send Email',
        ...
        validate: (p) => {
            if (!p.to || typeof p.to !== 'string')
                return 'Missing required param: to';
            if (!p.subject)
                return 'Missing required param: subject';
            if (!p.body)
                return 'Missing required param: body';
            if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(p.to))
                return 'Invalid email format: to';
            return null;
        },
    ...
    },
    {
        ...
        validate: (p) => {
            if (!p.action)
                return 'Missing required param: action';
            if (!p.properties)
                return 'Missing required param: properties';
            return null;
        },
        execute: async (params) => {
            return {
    ...
    },
    {
        ...
        validate: (p) => {
            if (!p.repo)
                return 'Missing required param: repo';
            if (!p.path)
    ...
    },
    {
        ...
        validate: (p) => {
            if (!p.query)
                return 'Missing required param: query';
            return null;
        },
        execute: async (params) => {
            return {
    ...
    },
    {
        ...
        validate: (p) => {
            if (!p.client)
                return 'Missing required param: client';
            if (!p.amount)
                return 'Missing required param: amount';
            if (!p.dueDate)
                return 'Missing required param: dueDate';
            return null;
        },
        execute: async (params) => {
            return {
    ...
    },
    {
        ...
        validate: (p) => {
            if (!p.taskId)
                return 'Missing required param: taskId';
            if (!p.durationMs)
                return 'Missing required param: durationMs';
            if (!p.label)
                return 'Missing required param: label';
            return null;
        },
        execute: async (params) => {
            return {
...
export async function executeIntegration(toolId, params, workspaceId, skipApproval = false) {
 ...
 */
export async function runActionImplementation(actionId, workspaceId) {
