...
import { executeIntegration, listTools, listPendingApprovals, resolveApproval, initWorkspacePermissions, grantPermission, } from './integrationManager.js';
...
void (async () => {
    ...
    for (const { userId, schedule } of persistedSchedules) {
        if (!schedule.intervalMs || !schedule.createdAt || !schedule.triggerType)
            continue;
...
app.post('/api/workspaces', async (req, res) => {
    const { userId, title, goalType = 'general', windowType = 'general', } = req.body;
    if (!userId || !title) {
...
app.post('/api/orchestrate', async (req, res) => {
    const { goal, userId = 'user_anonymous' } = req.body;
    ...
    try {
        ...
        const mode = req.body.mode || 'founder';
        // ── V2 TaskDAG Planning ───────────────────────────────────────────────
        emit({
            type: 'agent_spawn',
            taskId: 'planner',
...
app.get('/api/brain/missions', (_req, res) => {
...
app.post('/api/schedule/:scheduleId/pause', async (req, res) => {
    const userId = req.body?.userId;
    schedulerEngine.pauseSchedule(req.params.scheduleId);
    if (userId) {
        const schedule = schedulerEngine.getSchedule(req.params.scheduleId);
        if (schedule)
            await nexusStateStore.upsertSchedule(userId, schedule);
    }
    res.json({ success: true });
});
app.post('/api/schedule/:scheduleId/resume', async (req, res) => {
    const userId = req.body?.userId;
...
app.post('/api/tools/grant', (req, res) => {
    const { workspaceId, toolId } = req.body;
    if (!workspaceId || !toolId) {
        res.status(400).json({ error: 'Missing workspaceId or toolId.' });
        return;
    }
...
app.get('/api/approvals', (req, res) => {
    ...
    res.json({ approvals: listPendingApprovals(workspaceId) });
});
...
app.get('/api/fs/list', (req, res) => {
    ...
    const files = nexusFS.getFiles(parentId);
    const folders = nexusFS.getFolders(parentId);
...
app.get('/api/fs/search', (req, res) => {
    const { q = '' } = req.query;
    const results = nexusFS.searchFiles(q);
    res.json({ results });
});
// ── Boot ───────────────────────────────────────────────────────────────────
app.listen(PORT, () => {
